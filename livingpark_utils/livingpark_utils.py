...
class LivingParkUtils:
    ...
    def find_visit_pairs(self, months: int) -> dict:
        ...
        for (k, v), (k_, v_) in combinations(self.visit2month.items(), 2):
    ...
    def find_acceptable_visit_pairs(self, pd_df: pd.DataFrame, hc_df: pd.DataFrame) -> None:
        """Prints the number of unique PD-MCI, PD-NC and HC subjects per visit pairs.
        ...
        for c in combinations(events, 2):
            ...
            if len(v0 & v1):
                print(
                    ...
                    f" | Month difference: {self.visit2month[c[1]] - self.visit2month[c[0]]}"
                )
    ...
    def create_cohort(self, *, pd_df: pd.DataFrame, hc_df: pd.DataFrame, months: int) -> tuple:
        """Prints the number of unique PD-MCI, PD-NC and HC subjects per visit pairs.
        ...
        visit_pairs = self.find_visit_pairs(months)
        visit_df = pd_df.copy()
